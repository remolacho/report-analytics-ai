    ...
    module Gpt35Turbo
      class ActionsGraph
        ...
        <<~PROMPT
            ...
            3. Código debe ser válido, y debe crear nuevos df para que sean utilizados como en el ejemplo
            4. Respuesta en JSON

            IMPORTANTE: ejemplo de codigo
               df = df.select(columnas solicitadas).sum()
               df = Polars::DataFrame.new({
                  x: df.columns,
                  y: df.rows[0] })
               df.plot("x", "y", type: Es un String tipo "bar" o "line" o "pie" o "column")
